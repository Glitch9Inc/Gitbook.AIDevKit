---
description: >-
  A deep dive into SecureToken ??the XOR-based obfuscation system that keeps
  your keys out of plain sight
---
...
IL2CPP-compiled Unity builds can be partially reversed with tools like _Il2CppDumper_. Mono builds are even more accessible via _dnSpy_ or _ILSpy_. Once the attacker recovers the values of `M_PART_A` and `M_PART_B`, they can reconstruct the mask and decrypt any key stored in any asset file.
...
The goal of `SecureToken` is to eliminate _easy_, _automated_, and _passive_ extraction (file dumps, string searches, casual decompilation). It is not designed to stop a skilled, targeted reverse-engineer with full access to the binary.
...
See [Enterprise Proxy Server Security](../../../enterprise-proxy-server-security/).
